...
std::string handleOutputs_FPGA(UserFunction &func, std::stringstream &SSHostKernelParamList, std::stringstream &SSKernelParamList, std::stringstream &SSKernelArgs, bool strided, std::string index)
{
	if (func.multipleReturnTypes.size() == 0)
	{
		SSKernelParamList << " __global " << func.rawReturnTypeName << " * restrict skepu_output, ";
	...
	else
	{
		...
		for (std::string& outputType : func.multipleReturnTypes)
		{
			...
			SSKernelParamList << "__global " << outputType << "* restrict skepu_output_" << outCtr << ", ";
