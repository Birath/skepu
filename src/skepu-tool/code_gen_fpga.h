...
std::string generateOpenCLFPGARegion(size_t dim, UserFunction::RegionParam const& param);
...
IndexCodeGen indexInitHelper_FPGA(UserFunction &uf);


// void proxyCodeGenHelper_FPGA(std::map<ContainerType, std::unordered_set<UserFunction::RandomAccessParam const*>> containerProxyTypes, std::stringstream &sourceStream);
...
std::string handleOutputs_FPGA(UserFunction &func, std::stringstream &SSHostKernelParamList, std::stringstream &SSKernelParamList, std::stringstream &SSKernelArgs, bool strided = false, std::string index = "skepu_i");
